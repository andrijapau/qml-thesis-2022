...
class inference_circuit:
    ...
    class basis_encoding:
        ...
        def create_data_circuit(self, int_bin, float_bin, data_num=None):
            ''''''
            data_circuit = QuantumCircuit()

            if len(int_bin) != 0:
                int_reg = QuantumRegister(len(int_bin), 'x{}_int'.format(data_num))
            ...
            if len(float_bin) != 0:
                float_reg = QuantumRegister(len(float_bin), 'x{}_float'.format(data_num))
